...
import org.gradle.android.workarounds.room.commandlineproviders.JavaCompilerRoomSchemaLocationArgumentProvider
import org.gradle.android.workarounds.room.commandlineproviders.RoomSchemaLocationArgumentProvider
import org.gradle.android.workarounds.room.task.RoomSchemaLocationMergeTask
import org.gradle.android.workarounds.room.variants.ConfigureVariants
...
class JavaCompileWorkaround extends AnnotationProcessorWorkaround<JavaCompilerRoomSchemaLocationArgumentProvider> {
        ...
        androidVariantProvider.applyToAllAndroidVariants(project, new ConfigureVariants() {
            ...
            Closure<?> getOldVariantConfiguration() {
                return { variant ->
                    ...
                    variant.javaCompileProvider.configureWorkaroundTask { JavaCompile task ->
    ...
    void copyExistingSchemasToTaskSpecificTmpDir(Provider<Directory> existingSchemaDir, JavaCompilerRoomSchemaLocationArgumentProvider provider) {
        ...
        fileOperationsWorkaround.sync(existingSchemaDir, variantSpecificSchemaDir)
    }
    ...
    static void errorIfRoomSchemaAnnotationArgumentSet(Set<String> options) {
        if (options.contains(RoomSchemaLocationArgumentProvider.ROOM_SCHEMA_LOCATION)) {
            throw new IllegalStateException("""${RoomSchemaLocationWorkaround.class.name} cannot be used with an explicit '${RoomSchemaLocationArgumentProvider.ROOM_SCHEMA_LOCATION}' annotation processor argument.  Please change this to configure the schema location directory via the 'room' project extension:
    room {
